...
function calcularValoresParcela(parcela) {
  ...
  const multa = parcela.valorOriginal * MULTA_DIARIA_PERCENTUAL * atraso;
  return { atraso, multa, totalDevido: parcela.valorOriginal + multa, valorBase: parcela.valorOriginal };
}
  ...
  if (pagina === "detalheEmprestimo" && emprestimoSelecionado) {
    ...
    return (
      <Wrapper pagina={pagina} navegar={navegar}>
          ...
          <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, zIndex: 40, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <div style={{ background: BG_CARD, border: `1px solid ${BORDA}`, borderRadius: 16, padding: "1.5rem 1.25rem", width: "85%", maxWidth: 320, boxSizing: "border-box" }}>
              ...
              <p style={{ fontSize: 12, color: TEXTO_SEC, margin: "0 0 16px" }}>
                Valor original: {formatBRL(parcelaRenegociando.valorOriginal)}. Define o novo valor final — a multa automática deixa de ser aplicada nessa parcela.
              </p>
              <label style={labelStyle}>Novo valor final (R$)</label>
              <input
                type="number" value={valorRenegociacao} onChange={(e) => setValorRenegociacao(e.target.value)}
                placeholder="0,00" style={{ ...inputStyle, marginBottom: 14 }}
              />
              <button onClick={confirmarRenegociacao} disabled={!valorRenegociacao} style={{ ...btnPrimario, opacity: !valorRenegociacao ? 0.5 : 1, marginBottom: 8 }}>
                Confirmar renegociação
              </button>
              <button onClick={() => { setRenegociandoParcela(null); setValorRenegociacao(""); }} style={{ width: "100%", fontSize: 12, border: "none", background: "transparent", color: TEXTO_SEC }}>
                Cancelar
              </button>
            </div>
          </div>
        ...
        <div style={{ ...cardStyle, marginBottom: "1rem" }}>
          <p style={{ fontSize: 16, fontWeight: 600, margin: "0 0 4px" }}>{cliente ? cliente.nomeCompleto : "Cliente removido"}</p>
          ...
          <div style={{ borderTop: `1px solid ${BORDA}`, paddingTop: 10, display: "flex", justifyContent: "space-between" }}>
            <span style={{ fontSize: 13, color: TEXTO_SEC }}>Total devido hoje</span>
